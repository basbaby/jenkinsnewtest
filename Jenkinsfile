   ...
   stages {
        	...
        	steps {
					...
					container_Up = false
                    			timeout(time: 1, unit: "HOURS") { 
                        			sh "curl -u admin:admin -X GET -H \"Accept: application/json\" http://104.248.169.167:9000/api/qualitygates/project_status?projectKey=com.mycompany:jenkinsnewtest > status.json"
	 ...
	 stage('Build') {
        ...
        stage('Deploy to Cloudhub'){
        	steps {
				 withCredentials([file(credentialsId: 'settings', variable: 'settings')]){
        	    	sh "mvn -f pom.xml -s $settings package deploy -DskipTests -Dusername=basilbaby1310 -Dpassword=Basbaby1310 -DapplicationName=jenkinsnewtest -Denvironment=Sandbox -DmuleDeploy -Dcloudhub.region=us-east-2 -Danypoint.platform.client_id=12121 -Danypoint.platform.client_secret=21221 -Dkey=mule"
		...
		stage('Integrate Grafana'){
			steps{
				sh "curl --location --request POST 'http://46.101.7.76:8087/logs'  --header 'Content-Type: application/json' --data-raw '{\"username\": \"basilbaby1310\",\"password\": \"Basbaby1310\",\"orgName\": \"NJC POC\",\"envName\": \"Sandbox\", \"appNames\": [\"jenkinsnewtest\"]}'"
			}
		}	
